...
def get_stats():
    def as_query(alias_username):
        return ' '.join(('{alias} AS ('.format(alias=alias_username[0]),
                         'SELECT "artifactLocation",\n',
                         '        category as "category_{alias}"\n'.format(alias=alias_username[0]),
                         '        FROM categorise_tbl\n',
                         "        WHERE username = '{username}')".format(username=alias_username[1])))
    ...
    usernames = pd.read_sql(
        ...
        'FROM categorise_tbl', engine).set_index('username').index
    ...
    idx_username = tuple(map(lambda idx_username: (username2alias.get(idx_username[1],
                                                                      'q{c:02d}'.format(c=idx_username[0])),
                                                   idx_username[1]),
                             enumerate(usernames)))
